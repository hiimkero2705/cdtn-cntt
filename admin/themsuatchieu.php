...
while ($row = mysqli_fetch_assoc($result_phim)) {
    $phimoption .= "<option value='" . $row['IDPhim'] . "'>" . $row['TenPhim'] . "</option>";
}

// Khởi tạo một mảng để lưu trữ các tùy chọn cho trường select
$options = "";
while ($row = mysqli_fetch_assoc($result_rap)) {
    $options .= "<option value='" . $row['IDRap'] . "'>" . $row['TenRap'] . "</option>";
}
...
if (isset($_POST['submit'])) {
    ...
    $idphim = $_POST['idphim'];
    $idrap = $_POST['idrap'];
    $gioichieu = $_POST['gioichieu'];
    $ngaychieu = $_POST['ngaychieu'];
    // Thêm dữ liệu vào cơ sở dữ liệu
    $sql = "INSERT INTO suatchieu (IDSuat, IDPhim, IDRap, GioChieu, NgayChieu) 
            VALUES ('$idsuat', '$idphim', '$idrap', '$gioichieu', '$ngaychieu')";
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="row bg-secondary rounded align-items-center justify-content-center mx-0">
                    ...
                    <div class="bg-secondary rounded h-100 p-4">
                        <h2 class="mb-4 text-center">Thêm Suất Chiếu</h2>
                        <?php if (isset($msg))
                            echo $msg; ?>
                        <form method="post" enctype="multipart/form-data">
                            <div class="mb-3">
                                <label for="idsuat" class="form-label">ID Suất chiếu</label>
                                <input type="idsuat" class="form-control" name="idsuat" aria-describedby="idPhim">
                            </div>
                            <div class="mb-3">
                                <label for="idphim" class="form-label">Phim</label>
                                <select class="form-select mb-3" name="idphim" aria-label="Default select example">
                                    <?php echo $phimoption; ?>
                                </select>
                                <div class="mb-3">
                                    <label for="idrap" class="form-label">Rạp chiếu</label>
                                    <select class="form-select mb-3" name="idrap" aria-label="Default select example">
                                        <?php echo $options; ?>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label for="gioichieu" class="form-label">Giờ chiếu</label>
                                    <input type="time" class="form-control" name="gioichieu"
                                        aria-describedby="gioichieu">
                                </div>
                                <div class="mb-3">
                                    <label for="ngaychieu" class="form-label">Ngày chiếu</label>
                                    <input type="date" class="form-control" name="ngaychieu"
                                        aria-describedby="ngaychieu">
                                </div>
                                <button type="submit" class="btn btn-success m-2" name="submit">Thêm Suất</button>
