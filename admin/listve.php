...
$sql_count = "SELECT COUNT(*) AS total FROM ve";
$result_count = mysqli_query($conn, $sql_count);
$row_count = mysqli_fetch_assoc($result_count);
$total_ve = $row_count['total'];

// Số phim trên mỗi trang
$ve_per_page = 10;

// Tính số lượng trang
$total_pages = ceil($total_ve / $ve_per_page);

// Xác định trang hiện tại
$current_page = isset($_GET['page']) ? $_GET['page'] : 1;

// Xác định vị trí bắt đầu của phim trên trang hiện tại
$start_index = ($current_page - 1) * $ve_per_page;

$sql_ve = "SELECT * FROM ve JOIN Phim ON ve.IDPhim = Phim.IDPhim
                                JOIN rap ON ve.IDRap = rap.IDRap
                                JOIN suatchieu ON ve.IDSuat = suatchieu.IDSuat 
                                LIMIT $start_index, $ve_per_page";
$result_ve = mysqli_query($conn, $sql_ve);

$ve_array = array();
while ($row_ve = mysqli_fetch_array($result_ve)) {
    $ve_array[] = $row_ve;
}
                ...
                <div class="row bg-secondary rounded align-items-center justify-content-center mx-0">
                    <div class="col-12">
                        <div class="bg-secondary rounded h-100 p-4">
                            <h2 class="mb-4 text-center">Danh sách vé</h2>
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php foreach ($ve_array as $key => $ve) { ?>
                                            <tr>
                                                <th scope="row">
                                                    <?php echo $ve['IDVe']; ?>
                                                </th>
                                                ...
                                                <td>
                                                    <?php echo $ve['GioChieu'] . " vào " . $ve['NgayChieu']; ?>
                                                </td>
                                                <td>
                                                    <button class="btn btn-square btn-outline-danger m-2 delete-button"
                                                        data-id="<?php echo $ve['IDVe']; ?>"><i
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    deleteButtons.forEach(function (button) {
                        button.addEventListener('click', function (event) {
                            ...
                            if (!confirmDelete) {
                                event.preventDefault(); // Prevent the default action (deleting) if user cancels
                            } else {
                                var veId = this.getAttribute('data-id');
                                window.location.href = 'xoave.php?id=' + veId; // Redirect to xoave.php with the ID parameter
                            }
                        });
                    });
                });
